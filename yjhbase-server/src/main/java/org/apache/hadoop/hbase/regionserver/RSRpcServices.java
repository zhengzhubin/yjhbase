...
import org.apache.hbase.thirdparty.com.google.protobuf.*;
...
public class RSRpcServices implements HBaseRPCErrorHandler,
   ...
   */
  private boolean checkAndRowMutate(final HRegion region, final List<Action> actions,
  ...
  private void doAtomicBatchOp(final RegionActionResult.Builder builder, final HRegion region,
                               final OperationQuota quota, final List<Action> mutations,
  ...
  private void doBatchOp(final RegionActionResult.Builder builder, final HRegion region,
    ...
    try {
       ...
       */
      Map<Mutation, Action> mutationActionMap = new HashMap<>();
      int i = 0;
  ...
  private void closeAllScanners() {
    ...
    for (Entry<String, RegionScannerHolder> e : scanners.entrySet()) {
      try {
        e.getValue().s.close();
      } catch (IOException ioe) {
